...
##
##   xvfb-run -a godot --path game res://introshot.tscn -- <out-dir> [tab]
##
...
func _process(_delta: float) -> void:
	...
	image.save_png("%s/intro-%s.png" % [_out, _tab if _tab != "" else "how"])
	print("wrote %s/intro-%s.png" % [_out, _tab if _tab != "" else "how"])
	get_tree().quit()
